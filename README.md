...
UDP의 경우 커넥션이 3000개까지 테스트 결과 문제가 없지만 TCP의 경우 1000개 이상의 커넥션의 경우 Net::Socket의 vtable 주소가 변조되는 오류가 발생함  

![Alt text](./images/cloud.png)
외부의 vultr 클라우드 서버를 통해 3000개의 TCP 연결을 맺었을 때 오류가 발생하지 않는 것으로보아 내부 wsl2의 네트워킹 또는 메모리 등의 다양한 이슈로 확장될 수 있음을 파악