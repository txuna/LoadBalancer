...
# Nginx Reverse Proxy와의 대결
로컬호스트가 아닌 3개의 클라우드 서버를 구매하여 하나의 서버는 로드밸런싱 서버, 나머지 2개의 서버는 Flask서버로 속도 테스트를 진행해볼 예정이다.  
서버의 성능은 우분투 22.04, 1vCPU, 2 GB Memory, 3 TB Bandwidth이다.  
...
|------|---|---|
|요청 10개|2.23초|1.57초|
|요청 100개|33.80초|15.99초|
|요청 300개|80.23초|47.61초|

### 나의 로드밸런서 처리속도
|비고|서버 1대|서버 2대|
|------|---|---|
|요청 10개|2.78초|1.60초|
|요청 100개|29.90초|16.12초|
|요청 300개|101.23초|49.83초|


### Nginx 성공률
TCP 10000개의 요청을 벌스트하게 보냈을 때이다. 이때 Flask서버는 요청당 90K의 데이터를 전송한다.
...
# Comment 
로드밸런서 테스트 결과 추가되는 서버수에 따라 1개에서 2개로는 2배 이상의 시간을 절약하지만 TCP와 UDP 모두 3개 4개 이상의 경우 시간적인 효과는 확인할 수 없었음. 또한, 상용 프로그램인 Nginx와 TCP 커넥션 300개까지는 비비는 것을 보아 뿌듯함. 하지만 300개 이상으로는 디스커넥션이 자주 발생하였음.  
